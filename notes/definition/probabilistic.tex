...
We can simulate the sample function with the following algorithm. First we define the ranges of each token storing their upper end and then we call the function sample starting with $s$ being $0.\_$:
...
Its important to notice that the expected number of bits, although in the worst case it could be exponential, its constant, only depends on the number of tokens. This is because the probability of reaching the $k$-th recursive call its the probability of matching any border of the ranges so its $\sum_{i=1}^{\mid \Sigma \mid} \frac{1}{2^k} = \frac{\mid \Sigma \mid}{2^k}$. Lets call $X$ the random variable representing the number of recursive steps:
