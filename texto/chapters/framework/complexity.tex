...
\begin{definition}[Boolean circuits] 
    ...
    The size of $C$, denoted by $|C|$, is the number of vertices in it.\bigcifu{Robale un dibujo al Arora jaja}
...
\rafa{en casos como este que el subjet de mi def va en mathsf no sé cómo ponerlo en negrita quizás la jugada es sacarle el mathsf para la def?}\cifu{No se que decirte xD}
...
\bigcifu{Podrías poner acá que esta clase es igual a P}
...
For example, addition and multiplication of numbers of fixed precision can be done with $\AC^0$ circuits.\bigcifu{No termino de entender a qué problema te referís acá con el fixed precision.}
...
\end{definition}
\bigcifu{Ojo que ese $e$ de la definición tenés que usarlo multiplicativamente. Para mi lo mejor es definirlo como "La cadena tiene mas de  $(1/2 + e)n$ 1s o bien lo mismo pero con 0s. Luego, con $e= 0$ recuperás el original, y el teorema de Viola dice que para cualquier $e > 0$ el problema está en AC0.}

An interesting result from descriptive complexity is that for all $e$, there is a circuit in $\AC^0$ recognizing $\PROMISEDMAJORITY{e}$ \citep{viola2009approximate}.
...
\bigcifu{Idem dibujo xD. Seguro podes robarlo de algún lado. Pienso un coso que toma dos registros: uno con la entrada y otro que diga "random seed" o algo asi.}
...
\bigcifu{Se puede aclarar que la constante 2/3 acá es irrelevante, y que más adelante vamos a ver que la constante en la definición de transducers probabilísticos también lo es.}
...
\begin{theorem}
    ...
    For example $\AC^0 \subseteq \BPAC^0$.\bigcifu{Me parece bien este nombre, pero diría que es no estándar si es que no es estándar. Aparte, porque hay algo llamado $BP \oplus NP$ (o en general, podés poner $BP \oplus$ adelante de muchas clases) y puede mezclarse.}
\end{theorem}
...
This is because in $\AC^0$ we can enumerate all possible values for the random bits.\bigcifu{Qué valor de $f$? Podés tener problemas con el espacio, no? También hay algo menor, y es que dependiendo del autor a veces se asume que el fan-in arbitrario en realidad es fan-in polinomial respecto de la entrada.}
...
Adleman proved that the non-uniformity of $\ppoly$ gives enough power to simulate the randomness of $\BPP$. \rafa{hace falta agregar cita a Adleman?}
\bigcifu{A mi me gusta que todo tenga cita, pero no hace falta creo. Btw, acá también podés poner que P/poly es igual que BPP/poly. No se si tiene nombre la demo, podés ponerla en un apéndice. Aprovecho para recordar que movamos todas las demos a los apéndices para que los jurados lean esto rápido jaja.}
