...
It is a simpliﬁed model of the silicon chips used to make modern computers. 
...
\begin{definition}[Boolean circuits] 
    ...
    If $C$ is a Boolean circuit, and $x \in \{0,1\}^n$ is some input, then the output of $C$ on $x$, denoted by $C(x)$, is deﬁned in the natural way. 
    More formally, for every vertex $v$ of $C$, we give it a value $val(v)$ as follows: 
    If $v$ is the $i$-th input vertex then $val(v) = x_i$ and otherwise $val(v)$ is deﬁned recursively by applying $v$'s logical operation on the values of the vertices connected to $v$. 
...
The class $\ppoly$ ﬁts rather awkwardly in the complexity world since it contains undecidable languages such as the language Unary halt\footnote{UHALT = \{$1^n$ : $n$'s binary expansion encodes a pair $\langle M, x \rangle$ such that $M$ halts on input $x$\}}. 
The root of the problem is that for a language $\gL$ to be in $\ppoly$, it sufﬁces that a circuit family for $\gL$ exists even if we have no way of actually constructing the circuits.
Thus, it may be fruitful to try to restrict attention to circuits that can actually be built, say using a fairly efﬁcient Turing machine.
...
\begin{definition}[$\AC$]
    The class $\AC^i$ is deﬁned similarly to $\NC^i$ except gates are allowed to have unbounded fan-in (i.e., the $\OR$ and $\AND$ gates can be applied to more than two bits). 
...
\begin{definition}[$\TC$]
    The class $\TC^i$ is deﬁned similarly to $\AC^i$ except the circuits can have gates $\OR$, $\AND$, $\NOT$ and $\MAJORITY$ of unbounded fan-in.
...
\begin{definition}[$\MAJORITY$ with promise]
    For every $e \in (1/2, 1]$ we define the problem Majority with promise ($\PROMISEDMAJORITY{e}$) as $\MAJORITY$ but restricted to words where there are at least $e$ $0$s or $e$ 1s.
\end{definition}
...
\begin{definition}[Randomized Boolean circuits]
    A randomized Boolean circuit is a directed acyclic graph with $n+r$ sources and one sink.
    ...
    If $C$ is a Boolean circuit, and $x \in \{0,1\}^n$ is some input, then the output of $C$ on $x$, denoted by $C(x)$, is deﬁned as random variable in the following way.
    For every vertex $v$ of $C$, we give it a value $val(v)$ as follows: 
    If $v$ is the $i$-th input vertex, then $val(v) = x_i$; if $v$ is labeled as $\mathsf{random}$, then $val(v)$ is a uniform random Boolean variable; and otherwise $val(v)$ is deﬁned recursively by applying $v$'s logical operation on the values of the vertices connected to $v$. 
    The output $C(x)$ is the value of the output vertex.
\end{definition}
