...
For example, addition and multiplication of numbers of fixed precision can be done with $\AC^0$ circuits.\bigcifu{No termino de entender a qué problema te referís acá con el fixed precision.}\bigrafa{fixed precision = punto fijo}
Also, the equality of two strings of bits can be done in $\AC^0$.
From the ability to compute equality it follows that any function $f$ with $O(\log(n))$ bits as input\bigcifu{Entiendo a qué te referís, pero la oración no tiene sentido (quién es $n$?). Hay que reescribir esto.} can be computed in $\AC^0$.
...
\bigcifu{Se usa TC en algún lado? Sino, la sacaría.}
...
\begin{theorem}[\citet{viola2009approximate}]
...
\bigcifu{Este subtítulo tiene otro estilo comparado con el resto, no?}
...
This value is fixed once the evaluation of the circuit starts.\bigcifu{Esto se usa?}
...
\bigcifu{Referencia la figura del circuito randomizado! Esta piola y ayuda.}
...
\begin{theorem}
    ...
    For example $\AC^0 \subseteq \BPAC^0$.\bigcifu{Me parece bien este nombre, pero diría que es no estándar si es que no es estándar. Aparte, porque hay algo llamado $BP \oplus NP$ (o en general, podés poner $BP \oplus$ adelante de muchas clases) y puede mezclarse.}\bigrafa{en complexity zoo hay muchas clases que se llaman BP[C] que son la versión bounded probability de otra clase C. A qué te referis vos con $BP \oplus C$?}
    \bigcifu{Ok, lo que digo capaz solo aplica a clases con juegos interactivos tipo NP (cuando  le pones el BP le agregas randomness al verifier). Dejemos como esta entonces.}
\end{theorem}
...
\begin{proof}
    ...
    Therefore, we can output $\PROMISEDMAJORITY{2/3}$.\bigcifu{Which is in ... due to Theorem...}
\end{proof}
...
\begin{corollary}\label{coro:bpppoly-equals-ppoly}
    $\BPppoly = \ppoly$
\end{corollary}
\bigrafa{no sé si me convence mi argumento tirado así no más. Creo que lo tendría que explicar un poco más. Vale la pena que lo haga? Siento que no va a ser lo suficientemente largo como para ameritar un apéndice pero si lo suficientemente largo como para ser molesto.}
\bigcifu{Coincido que hay algunas observaciones que te podrían hacer, pero para mi el argumento que tirás convence lo suficiente. Lo falopa es decir que si vale una igualdad entonces podes aplicar $/Poly$ en ambos lados. En este caso vale porque la inclusión de Adleman es via simulacion exacta del algoritmo (lo mismo que hablamos el otro dia), asi que tambien podes simular el algoritmo con la ayuda.}