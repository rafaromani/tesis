...
Observe that all components of the deterministic transformers are either indexing in a matrix or calculations in $\Floating$; thus, composing their circuits, Theorem \ref{theo:ac0-sim-det-transf} is obtained.
...
Observe that the fact that a single forward pass of a transformer can be simulated by an $\AC^0$ circuit implies that a transformer with $O(\log(n))$ steps of $\CoT$ can also be simulated by $\AC^0$.
