In this work we analyze transformers as language recognizers.
...
These iterations are called \textbf{chain of thought} \citep{wei2022chain}.

We split the process of generating the next token into two separate steps.
First, the computation of the probability distribution over the alphabet which we call \textit{distribution generation}.
Second, the selection of a token based on the probability distribution, which we call \textit{token selection}.
...
In the real models, these parameters are the ones computed during the training. 
...
In other words, the output of the $i$th step is $\sigma_{i} = \TF^{i}(\alpha) = \TF(\alpha\sigma_{1}\dots\sigma_{i-1})$ where $\sigma_j = TF^j(\alpha) \in \Sigma$ for all $1 \leq j < i$.
...
As said before, the transformer only halts when it prints a decision symbol.
...
All transformers considered in this work employ the one presented in \citet{toyota}.
...
The dimension is dependent on the size of the input word.\footnote{This is not the case for the real models, which have constant embedding size as the same model is used across all inputs. 
In this work, many results are relevant for the case in which we have a logarithmic growth in the embedding dimension in relation to input size, but nonetheless most of the results apply under the stronger hypothesis of constant size. 
Also, observe that considering the possibility of increasing the size of the embedding somewhat resembles the process over which LLMs increase their size month by month as more computational resources are dedicated to them.}


We use $d(n)$ to refer to the \textbf{embedding size} when the input word has size $n$.

We now describe each part of the transformer's architecture on detail:
...
For all $\sigma \in \Sigma$, we denote by $\theta_{\TE}(\sigma) \in \mathbb{R}^d$ the mapping of token $\sigma$. 
We encode $\theta_{\TE}(\sigma)$ as one of the columns of $\theta_{\TE}$.
...
For all positions of the tape $1 \leq i \leq \budget(n) + n$ we denote by $\theta_{\PE}(i) \in \Rd$ the mapping of position $i$.
As in the token embedding, we encode $\theta_{\PE}(i)$ as the $i$-th column of $\theta_\PE$.
...
This layer is defined only as a single head attention layer because a multi-head attention layer with any constant number of heads can be simulated with multiple single-head attention layers \citep{cookbook}.\footnote{The matrices are named Q for query, K for key, V for value and O for output.
It is called query because $q_i$ represents a question of how much the $i$-th vector should attend to the others in a specific setting (determined by $W_Q$).
The $k_j$ vector answers said question such that $\langle q_i, k_j \rangle$ is large if the $j$-th vector is relevant to the $i$-th.
Thus, the key matrix is used to compute the relevance.
The value matrix has the role of extracting the information that the $j$-th vector should give to the $i$-th.
Finally, the output matrix is just a post-processing.
}
...
In contrast, the probabilistic selector operates stochastically.
It returns a token $\sigma \in \Sigma$ with probability $p(\sigma)$.
We allow for more than one maximum with this selector. Figure \ref{diag-transf} shows how the token selector connects with the distribution generator and its components.


\begin{figure*}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{chapters/diagrams/diagram-transf.png}
    \caption{Transformer architecture}\label{diag-transf}
\end{figure*}
...
Although the presentation for transformers introduced in this section operates over real numbers, the transformers analyzed during this work utilize $\Floating$ (a finite set of representable numbers, that we will define later) for representing them.
The formal construction is the same, but all operations over the set of real numbers are replaced by their corresponding in the set of $\Floating$ numbers defined in Section \ref{repr}.

Our model strikes a balance between fidelity to real-world models and simplicity.
For example, we do not model the normalization that transformers apply to vectors between layers.
In future work, it will be interesting to demonstrate that operations such as these do not affect the power of transformer abstractions.