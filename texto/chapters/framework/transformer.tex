...
The dimension is dependent on the size of the input word. \bigcifu{Aclararía que en la vida real no es así. Es parte de nuestro modelado.}
...
\textbf{Self Attention Layer: } \bigcifu{Idem lo de Santi, estaria bueno una intuicion, la tipica del diccionario supongo.} It is a mapping from $(\Rd)^a$ to $(\Rd)^a$.
...
This layer is defined only as a single head attention layer because a multi-head attention layer with any constant number of heads can be simulated with multiple single-head attention layers \citep{cookbook}.
...
The $\OUTPUT$ layer and the $\ATTN$ layer use a $\softmax$ function $\mathbb{R}^n$ to $\mathbb{R}^n$ defined as $\softmax(x)_i = exp(x_i) / \sum_{j=1}^n exp(x_j)$. \bigcifu{Reminder de qué hay que atajar los problemas con $-\infty$ en algún lado.}
...
In contrast, the probabilistic selector works non-deterministically.\bigcifu{Mejor digamos stochastic, para no pisarnos con cosas de no-determinisimo más en el sentido de complejidad.}
...
\begin{figure*}[h]
    ...
    \caption{Transformer architecture\bigcifu{Buen dibujo. Agregar una referencia desde el main text.}}
\end{figure*}
...
\bigcifu{No creo, asi como está se entiende. Lo que sí, falta definir con detalle ese $\Floating$. Sino, decir que lo vas a explicar más adelante en la seccion whatever.}