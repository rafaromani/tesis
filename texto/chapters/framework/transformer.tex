...
\bigcifu{No se si banco tantos renglones en blanco, pero whatever.}
...
\bigcifu{Where $\sigma_j = TF^j(\alpha) \in \Sigma$ for all $1 \leq j < i$.}
...
The dimension is dependent on the size of the input word.\footnote{This is note the case in the real models since the same model is used for all input sizes.}\bigcifu{Algún comentario más para defender la footnote? Tipo, decir que algunos resultados valen para el caso constante, o que de todas formas esta bueno tenerlo de parametro para entender tradeoffs.}
...
For all $\sigma \in \Sigma$, we denote by $\theta_{\TE}(\sigma) \in \mathbb{R}^d$ the mapping of token $\sigma$. \bigcifu{Capaz una aclaración de que $\theta(\sigma)$ is la columna $\sigma$ de $\theta$ (onda, como se codifica el mappeo).}
...
Given parameters $\theta_{\ATTN} = (W_Q, W_K , W_V , W_O) \in \mathbb{R}^{d \times d} \times \mathbb{R}^{d \times d} \times \mathbb{R}^{d \times d} \times \mathbb{R}^{d \times d}$, the layer is defined by the algorithm \ref{alg:def_attn}\bigcifu{Cuando poner refencias a cosas suele ser en mayuscula. Tipo Algorithm 1.}. 
...
All these parts come together for the distribution generator process in the algorithm \ref{alg:def_p_gen}.\bigcifu{Idem antes}
...
Although the presentation for transformers introduced in this section operates over real numbers, the transformers analyzed during this work utilize $\Floating$ \bigcifu{Agregaría una aclaración sobre qué son estos, tipo "(a finite set of representable numbers, that we will define later)"}for representing them.
