...
It is called query because $q_i$ represents a question of how much the $i$-th vector should attend the others in a specific setting (determined by $W_Q$).
The $k_j$ vector answers said question such that $\langle q_i, k_j \rangle$ is large if the $j$-th vector is relevant to the $i$-th.
Thus, the key matrix is for computing the relevance of the vectors.
...
In contrast, the probabilistic selector works stochastic.
It returns a token $\sigma \in \Sigma$ with probability $p(\sigma)$.
We allow for more than one maximum with this selector.

\medskip

In Figure \ref{diag-transf} there is a visual representation of how the token selector connects with the distribution generator and its components.
...
The formal construction is the same, but all operations over the set of real numbers are replaced but their corresponding in the set of $\Floating$ numbers defined in Section \ref{repr}.