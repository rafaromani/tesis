...
In most of the literature, the transformers operate over an infinite tape.
...
It's important to remark that the size of the tape bounds the number of steps of $\CoT$ that the transformer makes.
For an input word of length $n$, no computation can take more than $\budget(n)$ steps because every step write a token in the tape.\santi{los pasos son del cómputo, nodel transformer. Revisar en todo el texto}
\bigsanti{mejorar esta última oración, no se entiende}
\bigrafa{ahí va mejor?}
...
The algorithm works over vectors of numbers \santi{of reals?}\rafa{son $\Floating$. Puedo decir simplemente que son números por ahora?} of certain dimension $d$.
...
%Each part
\textbf{Token Embedding:} It's a mapping from $\Sigma$ to vectors in $\Rd$. 
...
\textbf{Position Encoding:} It's a mapping from the positions of the tape to vectors in $\Rd$.
...
\textbf{Self Attention Layer: } It's a mapping from $(\Rd)^a$ to $(\Rd)^a$. \santi{¿qué es $a$?}\rafa{es la cantidad de elementos que hay en la cinta en ese paso. Hay un vector por cada elemento. Cada paso de $\ATTN$ consume todos los vectores y devuelve la misma cantidad de vectores. En el algoritmo aparece $a$. No se me ocurrió otra forma para notarlo.}
...
This layer is defined only as a single head attention layer because we can simulate 1 multi-head attention layer with any constantly many heads with multiple single-head attention layers\cite{cookbook}.
...
\textbf{Feed Forward Network: } It's a mapping from $\Rd$ to $\Rd$. Given $\theta_{\FF} = (W_1 , b_1 , W_2 , b_2) \in \mathbb{R}^{d \times d} \times \Rd \times \mathbb{R}^{d \times d} \times \Rd$, the layer is defined as $\FF_{\theta_\FF}(h) := W_2 \; \relu(W_1 h + b1 ) + b2$, where $\relu: \mathbb{R}^d \to \mathbb{R}^d$ replaces each position $x_i$ by $\text{max}(0, x_i)$.
\bigsanti{Too many it's it's... Cambié algunos. Mejorar}

\textbf{Output Layer: } It's a mapping from $\Rd$ to a probability distribution over $\Sigma$.
...
\santi{sería mejor llamarlos "input" y "output"?}\rafa{a qué te referís?}
\begin{algorithm}
...
\bigrafa{Creo que formalmente es una familia de funciones porque es distinta para cada tamaño de vector, pero en el fondo hace siempre lo mismo. Quizás borrando el para todo $n$ queda más claro? No creo que valga la pena introducir que es una familia.}
...
Let $p : \Sigma \to [0,1]$ \santi{$\Sigma^*?$ } \rafa{No, el token selector consume la distribución que dado un token devuelve su probabilidad}be the output of the distribution generator.
In this work we study two selectors, we call them \textbf{deterministic selector} and \textbf{probabilistic selector}.
The deterministic selector always returns the token with the largest probability ($\argmax$ over $p$).
The probabilistic selector works non-deterministically.
It returns a token $\sigma \in \Sigma$ with probability $p(\sigma)$.


\bigrafa{Add an image representing the transformer architecture}
