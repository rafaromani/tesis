...
The dimension is dependent on the size of the input word.\footnote{This is note the case in the real models since the same model is used for all input sizes. In this work, we mainly worry about a logarithmic growth in the embedding dimension in relation to input size. In practice, we were unable to train models capable of processing inputs of arbitrary length, so accepting a small increase in the embedding dimension could still yield realistic results.}
...
As in the token embedding, we enconde $\theta_{\PE}(i)$ as the $i$-th column of $\theta_\PE$.
