...
For an input word of length $n$, no computation can take more than $\budget(n)$ steps because every step writes a token in the tape.\santi{los pasos son del cómputo, nodel transformer. Revisar en todo el texto}
...
We denote by $\TF(\alpha)$ the token outputted by one step of the transformer $\TF$ when the tape content is $\alpha \in \Sigma^*$.
