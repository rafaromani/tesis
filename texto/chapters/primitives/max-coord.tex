We will see later on that we will need to be able to take the maximum of a set of coordinates.\bigsanti{"we will see later on that we will need to be able to" es demasiado recargado. Podés decir "We show how to take the maximum of a set of coordinates, which will be useful for our constructions."}
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
\bigsanti{Ok, pero esto no da exactamente lo que se buscaba, ¿no?}

\subsubsection*{Implementation of the special operation}

The operation we are looking for to do it's \santi{gramática. Pasar revisor. No marco nada de redacción o gramática}not a linear transformation, therefore we need to  define a new technique.
...
\begin{align*}
    ...
    \end{matrix}\right) 
\end{align*}
\santi{¿qué eran $b_1$ y $b_2$? No es el mismo $b$ del $a,b$.}
...
The third \santi{third?}$\FF$ layer will be:
