With a similar strategy to the section \ref{repeat-token}, with the $\PE$ we flag when a certain position is reached.
...
Notice that this primitive with the one described in section \ref{repeat-token}, 
prove that for every transformer $\TF$ in $\CoTtndn$ there is a transformer $\TF'$ in the same class that always consumes all of its budget.
