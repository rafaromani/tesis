...
\end{definition}
\bigsanti{veo que más adelante se habla de la "first condition" y la "second condition". Sería bueno numerarlas o darles in nombre para que sea más fácil referirse a ellas.}
...
\bigsanti{Decí que lo que sigue es la implementación de un algoritmo para normalizar cualquier transformer.}

\subsection*{Algorithm for normalizing} {\color{orange} not sure about title}

Let's see how to normalize any transformer.
Let's take a transformer and modify it to meet the first constraint.\santi{"first constraint" se puede cambiar por el número o nombre que le hayas dado a la primera condición en la definición. Lo mismo para la segunda.}
...
Using the decomposition just presented, and the strategy defined in the previous section for applying linear transformations \santi{acá podés ser más preciso, si ponés números a secciones}, we can change the matrix of the $\OUTPUT$ layer to be only a projection.
