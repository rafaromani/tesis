...
\bigrafa{más de $\log(\log(n))$ pasos de cot requiere demasiado azar para $AC^0$.
Pareciera que ya con $\log(n)$ pasos los probabilisticos son más poderosos}


\begin{corollary}
    $\RCoT[\log(\log(n)), \poly(n)] \subseteq \RAC^0[\log(\log(n))] = \AC^0$
    \rafa{falta proof de este corolario}
\end{corollary}


\bigskip

Since one forward pass of constant-depth, polynomial-embedding-dimension, and constant-precision probabilistic transformers can be simulated by a circuit in $\RAC^0$.
In the same way as Corollary \ref{det-poly-steps-in-ppoly}, stacking the circuits of $\RAC^0[1]$ for each step of $\CoT$, gives us a simulation for a constant-depth, polynomial-embedding-dimension, and constant-precision probabilistic transformers performing $\poly(n)$ steps of $\CoT$.
This circuit has polynomial size and uses a polynomial number of random bits, thus we have Corollary \ref{prob-poly-steps-in-rppoly}.
