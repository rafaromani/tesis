...
Since it is not known whether $\Ptime = \BPP$, we cannot deduce the uniform version of Theorem \ref{cotpoly-equals-rcotpoly}, we can only prove:
%
...
This result suggests that studying probabilistic transformers is particularly relevant in fine-grained complexity, given the widely held (though unproven) belief that randomness yields a polynomial advantage.
...
Theorems \ref{bpp-equals-urcot} (a), \ref{adleman} (b) and \ref{cotpoly-ppoly} (c) give the following relation:\[
...
\bigcifu{Excelente esta sección final. Lo único: no revisé que estén bien las distintas referencias que hacés. Capaz un LLM puede hacerlo igual.}