...
En este trabajo se van a definir los transformers como reconocedores de lenguajes.
Un transformer consta de una cinta de $n_{max}$ celdas de la que lee el input (escrito en las primeras celdas) y va escribiendo a derecha de lo ya escrito nuevos caracteres usando un generador de tokens.


% Explicar funcionamiento de generadores 
...
Cada capa de decodificación está compuesta por dos subcapas: una capa de atención (ATTN) y una red de propagación hacia adelante totalmente conectada por posiciones (\textit{a position-wise fully-connected feed-forward network}) (FF). \todo{¿Cómo digo eso en castellano?}

Cada transformer opera sobre un alfabeto $\Sigma$ con dos símbolos especiales para aceptar y rechazar (los cuales no pueden estar presentes en las palabras que toma como input)\todo{puedo sacar ese constraint no?} y está definido por los parámetros $\theta = (\theta_{TE}, \theta_{PE}, \{\theta_{ATTN}^{(l)}, \theta_{FF}^{(l)}\}_{l=0}^{L-1}, \theta_{output})$. \todo{¿Un comentario sobre que $\theta$ es lo que se entrena?}
...
% Hablar sobre cómo se genera el siguiente token
% Va de la mano del algoritmo con la estructura del generador
% Definición de CoT
