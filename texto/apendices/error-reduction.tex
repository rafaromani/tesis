...
By definition of the family $\{\TF_n\}_{n\in\mathbb{N}}$, \mbox{$\Pr[y_i = 1] > c > 1/2$}.
...
Finally, choosing $r \geq -\frac{4\ln(d)}{c'\delta^2}$, we have $e^{-\frac{\delta^2}{4}rc'} \leq d$, and therefore, $\Pr[Y < r/2] < d$.

Thus, doing $\left\lceil-\frac{4\ln(d)}{c'\delta^2}\right\rceil$ runs of $\TF_n^*(x)$ and computing the majority vote of the runs is enough to give the correct answer for $\gL(x)$ with probability at least $d$.
