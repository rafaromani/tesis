...
\begin{theorem}[Chernoff's bound]
    Let $y_1, \dots, y_r$ be independent Boolean random variables with $\Pr[y_i = 1] = c$\bigcifu{Ojo que acá deberías tener $\geq c$ para modelar lo que pasa con los TF} for every $1 \leq i \leq r$.
...
Applying Chernoff's bound, we require $\delta$ and $r$ such that\bigcifu{La primer desigualdad siempre es cierta si $\delta>0$, $c>1/2$.} $(1+\delta)rc > r/2$ and $e^{-\delta^2rc/4} \geq d$.
\bigcifu{Está mal esto. Fijate que la versión de Chebyshev que estás usando te dice que tan probable es que te pases por arriba del promedio (que es $rc$). Vos querés la que hace por abajo para alejarte de la frontera $r/2$.}
%
\begin{align*}
