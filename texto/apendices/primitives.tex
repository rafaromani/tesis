...
\[
    \widetilde{h_f} = \left( \begin{matrix} h_f    \\ 0^{d \times 1}       \end{matrix} \right) 
    \xrightarrow{\text{first $\ATTN$ layer}}
    \widetilde{h_f} = \left( \begin{matrix} 0^{d \times 1}     \\  Mh_f \end{matrix} \right)
    \xrightarrow{\text{second $\ATTN$ layer}}
    \widetilde{h_f} = \left( \begin{matrix} Mh_f \\ 0^{d \times 1}      \end{matrix} \right)
\]
...
Recall the definitions of $\mathbb{I}_a$ and $\mathbb{I}_b$.
$\mathbb{I}_a$ is computed as $\relu(b-a)$. 
It is the indicator function of $a>b$, therefore $\mathbb{I}_a$ equals $1$ if $a>b$ and $0$ otherwise.
Notice that it cannot occur that $a \neq b$ but $b-a = 0$ since the numbers near $0$ are more densely represented.
$\mathbb{I}_b$ indicates $b>a$ in the same way as $\mathbb{I}_b$.
...
Since we are replicating a token that was printed, i.e., it is not part of the input\bigcifu{Nunca puede serlo? Entiendo que en los casos de uso de la primitiva es uno de los signos reservados, pero en general creo que pueden escribir lo que quieran los TF. Si lo dice el main text, capaz lo podríamos recordar acá.}\bigrafa{Podría serlo pero eso se parchea más abajo. La primitiva hace que una vez que se imprime el token, el transformer lo repite infinitamente}, its corresponding vector is the last one.
...
This case can be handled by exploiting that for each input size, we have a different position encoding.
