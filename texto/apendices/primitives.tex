...
Then, in the second layer we move the result to the right place.\bigcifu{En el dibujo de abajo me parece que sería mas claro escribir un vector de dos componente $h_f$ y $0^{n\times 1}$. Me pasó leyendolo que me mezcle pensando que esto era un vector con todos los 
$h_1\ldots h_f$ de cada posición.}
...
This can be achieved by maintaining them as $0$ since the beginning or transformed to $0$ with the primitive described in Section \ref{const-fun}.

\medskip

Let us construct each of the $\ATTN$ layers.
As mentioned before, in both layers we will need for $\widetilde{h_f}$ to only attend to itself.
...
Note that, after these two layers, the values of $h_i$ for all $i \neq f$ have changed. Yet, this is not relevant with respect to the way we use the primitive.





\section{Implementation of the special operation and first step of maximum algorithm}\label{appendix:special-op}

The $\FF$ layer needed for the first step transforms $(a, b, 0, 0)$ in $(a, b, \relu(a-b), \relu(b-a))$.
It consumes $(a, b, 0, 0)$ and outputs $(0, 0, \relu(a-b), \relu(b-a))$ which added to $(a, b, 0, 0)$ gives what we need.
Instantiating the matrices as follows produces the desire effect:
...
The special operation is more complicated.
It is not a linear transformation; therefore, we need to define a new technique.
Observe that one\bigcifu{Estas son las indicadoras de que $a>b$ y al revés? Porque si está en el main text capaz está bueno recordarlo ahora. Aparte, habría que hacer algún comentario sobre precisión numérica? I.e. no puede pasar que $a\neq b$ pero $a-b = 0$? Supongo que con alguna hipótesis se arregla, suena también que es algo cierto dado que la representación numérica es más densa cerca del 0.} of $\mathbb{I}_a$ and $\mathbb{I}_b$ equals $1$ and the other equals $0$.
...
\bigcifu{Para mi está bueno. Hasta acá me parece que está super piola el apéndice.}
...
Since we are replicating a token that was printed, i.e., it is not part of the input\bigcifu{Nunca puede serlo? Entiendo que en los casos de uso de la primitiva es uno de los signos reservados, pero en general creo que pueden escribir lo que quieran los TF. Si lo dice el main text, capaz lo podríamos recordar acá.}, its corresponding vector is the last one.
...
This case can be handled by exploiting the non-uniformity of the model\bigcifu{No se si es la no-uniformity, porque este truco sale uniformemente. Capaz pondría que el caso general se parchea usando el positional embedding contando lo que pones abajo y listo}.
Since the input word has size $n$, we can use the $\PE$ to flag the first $n$ vectors.
